package leetcode.editor.cn;//有一幅以二维整数数组表示的图画，每一个整数表示该图画的像素值大小，数值在 0 到 65535 之间。
//
...
// 
// Related Topics 深度优先搜索 广度优先搜索 数组 矩阵 👍 254 👎 0
...
//            [1,1,0],
//            [1,0,1]]
// 一个坐标（1,1）和新数值（2），要求把坐标及其周围的旧数值变为新数值，
// 即新矩阵为：
//            [[2,2,2],
//            [2,2,0],
//            [2,0,1]]
        ...
        private int[][] dfs(int[][] image, int sr, int sc, int newColor, int i) {
            if (sr < 0 || sr >= image.length || sc < 0 || sc >= image[0].length || image[sr][sc] == newColor || image[sr][sc] != i) {
